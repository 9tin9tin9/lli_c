...
static inline size_t Vec_size(const Vec* vec) { return vec->size*vec->elem_size; }
...
static inline Vec Vec_copy(const Vec* src) {
    Vec v = *src;
...
static inline int _Vec_push(Vec* v_, const void* elem, size_t elem_size) {
    ...
    memcpy(_Vec_at(v_, v_->size-1), elem, elem_size);
    return 1;
}
...
// return 1 if success
static inline int _Vec_insert(Vec* v_, const Vec* a_, size_t p_) {
    // different elem sizes
