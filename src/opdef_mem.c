...
mov(const Vec* v, Mem* m, Signal* s)
{
    argcGuard(v, 2);
    double val;
    try(Tok_getValue(Vec_at(v, 1, Tok), m, &val));
    try(Tok_writeValue(Vec_at(v, 0, Tok), m, val));
    *s = Signal(None, 0);
    return Ok;
}
...
cpy(const Vec* v, Mem* m, Signal* s)
{
    argcGuard(v, 3);
    long des;
    try(Tok_getLoc(Vec_at(v, 0, Tok), m, &des));
    long src;
    try(Tok_getLoc(Vec_at(v, 1, Tok), m, &src));
    size_t size;
...
var(const Vec* v, Mem* m, Signal* s)
{
    ...
    HashIdx var;
    long idx;
    try(Tok_getSym(Vec_at(v, 0, Tok), &var));
    try(Tok_getLoc(Vec_at(v, 1, Tok), m, &idx));
...
#define mutVarIdx(v_, m_, s_, a_) \
    ...
    HashIdx var; \
    size_t incrVal; \
    try(Tok_getSym(Vec_at(v_, 0, Tok), &var)); \
    try(Tok_getUint(Vec_at(v_, 1, Tok), m_, &incrVal)); \
    long varIdx; \
